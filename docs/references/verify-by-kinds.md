...
- **The collector measures, the judge judges.** `collect-ac-evidence.py` never emits an AC pass/fail — only facts + a vacuity exit code. Coverage ("does this test actually cover the AC?") is the review judge's call, from the evidence + the test body. `expected_match: {min_count}` (default 1) lets an AC declare how many real tests must match (catches zero/under-match deterministically). A runner with a parseable format declares its `parser:` (`cargo-test.v1` / `pytest.v1` / `sh-tap.v1` — AE's `ok:`/`FAIL:` shell convention). **`exit_code_only: true` is a TOTAL vacuity bypass** (the exit code becomes the only signal — no match-count) — use it ONLY for a genuine single-gate command with nothing to count (a lint, a negative-assert), **NEVER as a shortcut around writing a `parser:`**, or F1 reopens. `matched_tests[].file`/`line` are **best-effort** (null for runners whose default output omits them — cargo/sh-tap); the Check 7 judge then resolves the test body by grepping the test name.
