...
| Filesystem safety | Full power-loss / mount / device qualification matrix | Ordinary no-clobber writes, staging, `fsync`, and honest failure detection. | These four cover the failures a normal session actually hits. Formal durability qualification moves to V5, driven by an observed failure. |
