...
state: fresh
anchors:
  - source: "plugins/ae/scripts/graph-lint.py:41"
    anchor_hash: "ID_RE = re.compile(r\"^(F-\\d+|BL-\\d+|disc-\\d+)$\")"
  - source: "plugins/ae/scripts/graph-page-check.py:154"
    anchor_hash: "verdict = \"DEFECT\" if defects else (\"stale\" if stale else \"fresh\")"
  - source: "plugins/ae/scripts/graph-refresh.py:56"
...
Judgment, unanchored: the graph persists two kinds of JUDGED content — and their mechanical by-products (the append-only mutation log, the regenerated index); everything semantic beyond these two derives at read time. Domain one is relationship edges between project artifacts, whose target-id universe is deliberately a closed three-prefix enum (plugins/ae/scripts/graph-lint.py:41) — which is why synthesis pages were kept OUT of the edge graph: admitting a fourth prefix means touching the target-dispatch chain and a new resolver set, so v1 pages are leaf-only. Domain two is synthesis pages — prose understanding whose every claim anchors into the working tree, machine-graded into exactly three ANCHOR-freshness states — the grading never claims semantic truth, which stays with the judge and the human (plugins/ae/scripts/graph-page-check.py:154).
