...
- [file:contains:plugins/ae/skills/roadmap/SKILL.md] zero-PROMOTE-with-Tier-1-CLOSE SKIPs the PROMOTE Step A/B entirely (no empty PROMOTE list, no PROMOTE AskUserQuestion) and goes straight to CLOSE confirmation
- [file:contains:plugins/ae/skills/roadmap/SKILL.md] the CLOSE `Remove some` multiSelect contains ONLY Tier-1 actionable CLOSEs (never Tier-2 advisory lines) and re-renders + re-confirms before moving (does not move on the multiSelect alone)
