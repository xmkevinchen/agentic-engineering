...
| Discuss loop | `test-discuss-loop-contract.sh` — the legs of the discuss stage's control flow: a seat that is told to write a file can write one, and every rule that spans a writer, a reader and a discharger keeps all of them |
