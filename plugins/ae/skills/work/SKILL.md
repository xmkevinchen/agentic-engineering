...
- [ ] **Record durable lessons** (F-070 — the graph carries relationships, not experience): write the shipped feature's experiential knowledge — key decisions, surprises, rework causes — to the assistant's persistent memory when the host provides one (e.g. CC auto-memory: a topic file + one MEMORY.md index line). Skip for trivial mechanical work with nothing worth remembering. This line is the lessons' prompt-level home; without it the reflection has no durable target (the plan/review artifacts record what was built, not what executing it taught).
