...
4. **Auto-pass gate** (default: ON) — evaluate after every step:
   ...
   - No test command → `tests_green` = UNVERIFIED. **Branch on the current step's AC `verify_by` values (F-041 harness contract)**:
     ...
     - All of the step's ACs are `judge`/`manual` → **check-only**: an empty `test.command` is expected (no deterministic claim to run); continue without the UNVERIFIED pause, leaving `judge` ACs to review-stage rubric-confirmation. **Judge-artifact producer (HDD)**: if a `judge` AC's rubric needs a captured runtime artifact (UI screenshot / perf metric / data sample), the executor MUST write it to `<milestone-dir>/artifacts/<AC-id>.<ext>` so `/ae:review` Check 7 evaluates the artifact (not a self-report); for code/doc/prose judge ACs the diff IS the artifact — no file needed.
...
- [ ] **Record durable lessons** (F-070 — the graph carries relationships, not experience): write the shipped feature's experiential knowledge — key decisions, surprises, rework causes — to the assistant's persistent memory when the host provides one (e.g. CC auto-memory: a topic file + one MEMORY.md index line). **Timing (integration finding)**: for a harness-carrying plan, run this AFTER the harness loop's terminal `exit_pass` finalize (the feature is only then actually shipped — recording at all-steps-done would freeze lessons the review loop may still overturn); only no-harness legacy plans record here directly. Skip for trivial mechanical work with nothing worth remembering. This line is the lessons' prompt-level home; without it the reflection has no durable target (the plan/review artifacts record what was built, not what executing it taught).
