...
2. Loop: `ready = sh plugins/ae/scripts/check-dag.sh <plan> ready <milestone-dir>/notes.md` → the ids whose `depends:` are all `pass`. Empty ready-set AND not-all-`pass` ⇒ a node is `blocked`/`fail` → surface + stop. Pick ONE ready node (serial; concurrent batch-spawn is Phase-2).
3. **Commit-before-execute**: write `NODE_STATE <id>: in_progress` to `<milestone-dir>/notes.md` BEFORE doing the node's work — so a crash/context-loss resumes from the ledger (sync from `notes.md`, not the plan) and never re-executes a node (no ghost execution).
4. Do the node's work; then `sh plugins/ae/scripts/check-node.sh <plan> <node> <iter> <cap>` re-derives the verdict FROM DISK. Write `NODE_STATE <id>: pass` (or `fail`) to the ledger.
5. Recompute ready; repeat until every node is `pass`.
- **Namespace rule (load-bearing)**: `NODE_STATE` is the orchestrator's scheduling/resume ledger ONLY. `check-node.sh` NEVER reads `NODE_STATE` — the verdict is ALWAYS re-derived from disk (files/git/test). A ledger entry is not evidence: a stale `in_progress`/`pass` line can never fake a node's real verdict (preserves the F-050 anti-hallucination guarantee).
