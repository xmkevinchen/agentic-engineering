...
**DAG mode (F-054 Phase-1 — opt-in)**: if the plan has `dag: true` frontmatter, current-step selection is NOT document order — it is the **ready-set frontier**. Drive (serial in Phase-1):
1. `sh plugins/ae/scripts/check-dag.sh <plan> validate` once at start — non-zero ⇒ refuse (malformed DAG; fix the plan).
2. Loop: `ready = sh plugins/ae/scripts/check-dag.sh <plan> ready <milestone-dir>/notes.md`. The command emits a **machine-distinguishable terminal signal** (no inference): a list of ready node ids (exit 0) ⇒ work available; `__DONE__` (exit 0) ⇒ every node is `pass`, finish + `/ae:review`; `__BLOCKED__` (exit 3) ⇒ nodes remain but a dependency failed → surface + stop. Pick ONE ready id (serial; concurrent batch-spawn is Phase-2).
3. **Commit-before-execute**: write `NODE_STATE <id>: in_progress` to `<milestone-dir>/notes.md` BEFORE doing the node's work — so a crash/context-loss resumes from the ledger (an interrupted `in_progress` node is re-picked by `ready`; sync from `notes.md`, not the plan) and never ghost-executes a completed node.
4. Do the node's work; then advance it via `sh plugins/ae/scripts/advance-node.sh <plan> <node-num> <id> <milestone-dir>/notes.md <iter> <cap>`. **advance-node.sh is the ONLY sanctioned writer of `NODE_STATE <id>: pass`** — it runs `check-node.sh` (verdict re-derived FROM DISK) and writes the ledger entry from its exit code alone. The loop/LLM **cannot hand-write a `pass`** — provenance is the check-node exit code, not the caller's word (closes the ledger-trust seam; F-049 anti-theater lesson applied at the DAG layer).
5. Recompute ready; repeat until `__DONE__`.
- **Namespace rule (load-bearing)**: `NODE_STATE` is the orchestrator's scheduling/resume ledger ONLY. `check-node.sh` NEVER reads `NODE_STATE` — the verdict is ALWAYS re-derived from disk (files/git/test). A ledger entry is not evidence: a stale `in_progress`/`pass` line can never fake a node's real verdict (preserves the F-050 anti-hallucination guarantee), and `pass` can only be written by `advance-node.sh` gating on a real check-node verdict.
