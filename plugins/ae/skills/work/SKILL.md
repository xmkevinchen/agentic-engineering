...
**DAG mode (F-054 — opt-in, thin-driver loop)**: if the plan has `dag: true` frontmatter, current-step selection is NOT document order — it is the **ready-set frontier**, driven by `dag-next.sh` (F-055). The deterministic orchestration (compute ready, pick, commit-before-execute, detect done/blocked) is done BY the driver; the TL's residual is only *doing the node's work* + advancing it.
1. `sh plugins/ae/scripts/check-dag.sh <plan> validate` once at start — non-zero ⇒ refuse (malformed DAG; fix the plan).
2. **Driver loop**: `out = sh plugins/ae/scripts/dag-next.sh <plan> <milestone-dir>/notes.md`. Dispatch on the one-line instruction (no inference):
   - `LEGACY` ⇒ fall back to linear document-order (this branch shouldn't fire on a `dag: true` plan).
   - `DONE` ⇒ every node is `pass` → finish + `/ae:review`.
   - `BLOCKED` (exit 3) ⇒ a node is `gate`-escalated and blocks the frontier → surface + stop for the human.
   - `NEXT <id> <step-num>` ⇒ the driver has ALREADY written `NODE_STATE <id>: in_progress` (commit-before-execute, automated — crash-resume re-picks it). Do that node's work (spec = its `### Step <step-num>` block), then advance it: `sh plugins/ae/scripts/advance-node.sh <plan> <step-num> <id> <milestone-dir>/notes.md <iter> <cap>`.
3. Repeat the driver loop until `DONE`.
- **advance-node.sh is the ONLY sanctioned writer of `NODE_STATE <id>: pass`** — it runs `check-node.sh` (verdict re-derived FROM DISK) and writes the ledger entry from its exit code alone. The loop/LLM **cannot hand-write a `pass`** (closes the ledger-trust seam; F-049 anti-theater at the DAG layer).
- **Namespace rule (load-bearing)**: `NODE_STATE` is the orchestrator's scheduling/resume ledger ONLY. `check-node.sh` NEVER reads `NODE_STATE` — the verdict is ALWAYS re-derived from disk (files/git/test). A ledger entry is not evidence (preserves the F-050 anti-hallucination guarantee).
- **Honest bound**: the driver shrinks the ignition turtle (every per-step orchestration decision is now mechanical) but does NOT remove it — the TL must still START the loop (a CC plugin cannot strip the main agent's control).
- A plan WITHOUT `dag: true` ⇒ today's linear document-order behavior, unchanged (no silent DAG interpretation of legacy plans).
...
4. **Auto-pass gate** (default: ON) — evaluate after every step:
   ...
   - **Per-node advancement (F-050 — replaces the implicit between-node "continue?" judgment)**: run `sh plugins/ae/scripts/check-node.sh <plan> <N> <iter> <cap>` for the current step N. It re-derives the verdict FROM DISK (deliverables present; never the agent's say-so) → **`pass` (exit 0)** = the node is auto + complete → advance automatically, NO human "continue?" prompt; **`gate` (exit 2)** = the node is a `human-gate` (judge/manual AC) OR the fixup cap is exhausted → **pause for human** (the only legitimate mid-flow pause), regardless of `auto_pass`; **`fail` (exit 1)** = a deliverable is missing → fixup, do not advance. This runs BEFORE `loop-decide.sh` (a `gate` bypasses the cap loop; `pass`/`fail` feed the existing terms). The gate expression gains a `node_verdict_pass` term: `gate = node_verdict_pass AND tests_green AND no_p1 AND ...`. The point: "should I stop and ask?" is no longer the agent's discretionary call — it is `check-node`'s exit code (gate→pause, pass→drive). **F-051**: if the step declares `node_check:` lines, `check-node.sh` additionally runs each through the hardened runner — a **red-before-green probe (`redcheck`) every iteration** (the instance must demonstrably fail on a harness-synthesized bad input) THEN the real check; a theater/invalid/failing `node_check` → `fail` (does NOT advance). This is a content gate on top of deliverable-presence; absent `node_check` ⇒ unchanged.
   - **Structural pause-authority (F-053 / BL-148)**: an **auto-node** (`human-gate: false`) step's implementation runs in the spawned **developer subagent** (the `Agent(...)` worker above) — and a subagent spawned via the Agent tool **structurally lacks `AskUserQuestion`** (a CC tool-availability fact, not prose: interactive prompting is a main-loop capability the worker does not have). So the worker **physically cannot pause-to-human mid-node**; it must return to the TL, who runs `check-node.sh` and drives the loop. This is the only NON-prose (structural) lever bounding mid-node pause-authority to the node boundary — it closes the *inside-node* tier of the F-050 ignition residual. **Honest bound (do NOT overclaim):** in **solo mode** (`AGENT_TEAMS_FULL=false` / no developer agents) the Lead/main agent performs node-work and RETAINS `AskUserQuestion` → the structural lever is ABSENT, a mid-node pause is possible (documented Known Limit, consistent with the solo-mode fresh-eyes-absent limit). And the *ignition turtle* (the TL must still START the loop) is a SEPARATE F-050 residual this does not touch — a CC plugin cannot strip the main agent's `AskUserQuestion`.
