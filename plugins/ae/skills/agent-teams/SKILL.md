...
**Agent NAMES are not the same case (do NOT generalize the above to them)** — a Task *subject* is a display label on a record that carries a harness-owned unique ID, so duplicate subjects are cosmetic. An agent *name* is the address key: `SendMessage(to: "<name>")` routes by that bare name. So two live-or-resumable agents sharing a name is an addressing hazard, not a display collision — the anti-over-engineering stance above ("don't pre-engineer a prefix scheme") still holds, but it does NOT license reusing a name while a prior same-name agent may still be live. **Cross-skill rule**: before a NEXT skill re-spawns a bare teammate name a prior skill used (e.g. `/ae:plan` spawning `codex-proxy` right after `/ae:discuss` tore its team down), confirm the prior team's `shutdown_response` / `TeamDelete` completed — ordered teardown, not assumed. Observed live: an `/ae:discuss` teardown overlapping the next skill's same-name spawn (F-042). Within a single skill, distinct per-phase names avoid the overlap outright (F-077); across skills, teardown ordering is the guard a single-file lint can't see.
