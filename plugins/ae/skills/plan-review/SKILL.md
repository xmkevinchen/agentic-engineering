...
- Every `judge`-typed AC declares `judge-class: fact-claim | form` (plan/SKILL.md's authoring rule). No class on a NEW plan → **Must fix** (legacy plans migrate on touch). **fact-claim rubrics get the strength check**: the rubric must name (a) a source set the judge reads BEFORE the artifact, (b) a claim-by-claim output shape (per-claim verdict + evidence cite + the judge's re-derived answer), (c) cross-family/fresh-context when the artifact is self-authored. A fact-claim rubric satisfiable by inspecting the artifact's self-consistency alone — no source set named, no claim-by-claim shape — → **Must fix** (that is the form-rubric-on-a-fact-claim failure: it passes confident-but-wrong content). A `form`-class AC with a one-question rubric passes untouched — no re-derivation tax on shape/quality checks.
...
- **Harness-readiness gate** — plan-review confirms each step's harness is IN PLACE + bites BEFORE `/ae:work` drives it. (a) Every `### Step N` declares `human-gate: true|false` consistent with its ACs (any `judge`/`manual` AC ⇒ must be `true`; all-deterministic ⇒ `false`); mismatch or missing on a feature-dir plan → **Must fix**. (b) Every **deterministic** `verify_by` AC (unit/integration/e2e/contract) MUST **declare** a `verify:` line — enforced mechanically: run `check-harness.sh <plan>`; non-zero exit → **Must fix** (it lists the deterministic ACs with no `verify:` line; `judge`/`manual` exempt). `check-harness.sh` is a **completeness lint** (the `verify:` line is *declared*) — NOT a coverage check; coverage is judged at `/ae:review` from the evidence record `collect-ac-evidence.py` emits. plan-review also sanity-checks `expected_match.min_count` if an AC declares it. A deterministic claim with nothing to run is vacuous — whether the step is auto or human-gated. (c) `human-gate:` is **derived** at plan time, so a mismatch usually means the AC kinds changed after derivation — re-derive, don't hand-patch.
