...
- **`human-gate: true | false` (per STEP, derived — F-050)**: at plan time `/ae:plan` derives each `### Step N` block's `human-gate` from that step's ACs' `verify_by` — **any `judge`/`manual` AC → `human-gate: true`** (a human node: the harness loop pauses here for human confirmation / feature-level review, does NOT auto-advance); **all-deterministic (`unit`/`integration`/`e2e`/`contract`) → `human-gate: false`** (an *auto* node: the loop advances on its mechanical check, `check-node.sh → pass`). Write the derived value as a `human-gate: true|false` line inside the step block. This is the disk annotation that `/ae:work`'s loop-engagement trigger reads: **a plan with ≥1 `human-gate: false` step ⇒ the harness loop drives; none ⇒ legacy behavior** (backward-compatible). Derived by default — do not require authors to hand-write it (the form is dynamic; the LLM fills it from the actual AC kinds, the crystallized frame consumes it).
- **`node_check: <template-id> k=v ...` (per STEP, optional — F-051)**: an auto-node MAY declare one or more `node_check:` lines inside its `### Step N` block to gate advancement on *content-correctness*, not just file presence. The LLM authors **structured data only** (a template id + whitespace-free `key=value` params) — never shell. A hardened runner (`run-node-check.sh`) validates `template ∈ plugins/ae/scripts/check-templates.catalog` + required params, then executes a human-authored predicate; `check-node.sh` runs a **red-before-green probe** (`redcheck`) first — the instance must demonstrably FAIL on a harness-synthesized bad input before its real pass is trusted (defeats tautology/`exit 0`/fixture-name-branching theater; the must-fail input is harness-generated, never LLM-authored). v1 catalog: `file-contains` (target,pattern), `json-field` (target,path), `command-output` (cmd,expect — cmd must be an allowlisted entry, NOT arbitrary shell). The catalog GROWS by a config entry; the authority surface does not (the floor-(3)/(4) boundary). Lifecycle (per-feature vs project) derives from the step's ACs' existing `fixture:` — **conservative rule for mixed-AC steps (F-052 integration): a step with ANY `fixture: project` AC ⇒ its `node_check`(s) are treated as `project`-lifecycle** (durable treatment wins when mixed, so the cross-family bite-review can never be silently dodged by pairing a project AC with a per-feature one). Absent `node_check` ⇒ today's presence-only behavior (backward-compatible). **Honest bound (do NOT overclaim):** `node_check` kills *authoring-surface* theater but does NOT close two residuals — (1) **semantic-depth Potemkin** (output side: "high-fidelity garbage" that passes a present-and-biting check; F-050 residual, unbeatable by any structural frame) and (2) **param-selection theater** (input side: the red-before-green probe proves a check *can* fail — falsifiability — not that its params exercise the real AC — relevance). Relevance is bounded only by plan-review param-scrutiny + cross-family review (BL-150), never fully closed.
- **`dag: true` + per-node `id:`/`depends:` (OPT-IN task DAG — F-054 Phase-1)**: a plan MAY opt into DAG execution by adding `dag: true` to its frontmatter. Then each `### Step N` (= a node) declares `id:` (stable, e.g. `N3`) and `depends: [N1, N2]` (multi-predecessor; absent ⇒ root). `/ae:work` then advances by the **ready-set frontier** (a node runs only when all its `depends:` are `pass`, verdict re-derived from disk) instead of document order — so long tasks advance autonomously on dependency order. Each node keeps its existing deliverable (`Expected files:`) + harness (`verify_by`/`node_check`); `check-dag.sh` validates the graph (acyclic, no dangling depends, every auto-node has a harness) and computes the ready-set. **Node `backend:` (if set) ∈ {`subagent`, `agent_teams`} ONLY — never `workflow`** (workflow is the orchestrator level, not a node backend; `check-dag.sh` rejects it). A plan WITHOUT `dag: true` is a normal linear plan (unchanged). **Phase-1 scope**: `id` + `depends` + ready-set **serial** drive only. NOT in Phase-1 (later phases, do not author them yet): `may_expand:` (runtime DAG growth), `backend:` derivation, `allow_overlap:`, concurrent execution. **Honest bounds (do NOT overclaim):** (1) this opt-in DAG reverses F-050's "plan IS the proto-DAG" on a *goal-driven* basis (long-task autonomy), not an observed failure — phased, each phase proven on a real long task first; (2) **Phase-1 is SERIAL, not parallel** (ready-set is computed but dispatched one node at a time; concurrent spawn is Phase-2, and even then speedup is modest under the serial TL — the win is autonomous advancement, not raw speed); (3) **the DAG is fixed at plan time in Phase-1** (runtime growth is Phase-3); (4) the DAG raises the floor + makes drift visible but does NOT guarantee semantic correctness — semantic-depth Potemkin remains uncloseable (F-050 residual).
