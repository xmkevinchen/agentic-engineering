...
- **Runnable-check mandate (HDD)**: every **deterministic** `verify_by` AC (`unit`/`integration`/`e2e`/`contract`) MUST declare a `verify:` line naming the command (e.g. `verify: sh tests/x.sh` — the command MAY invoke the project `test.command`). See [`docs/references/verify-by-kinds.md`](../../../../docs/references/verify-by-kinds.md) for what a runnable check looks like per kind. `/ae:plan-review` enforces this mechanically via `check-harness.sh` (it detects the `verify:` line; the command's *validity* is checked by `verify-ac.py` at run time). A deterministic claim with nothing to run is vacuous. `judge`/`manual` ACs are exempt (artifact/human, not shell).
