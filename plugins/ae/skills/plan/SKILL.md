...
- **`dag: true` + per-node `id:`/`depends:` (OPT-IN task DAG)**: a plan MAY opt into DAG execution by adding `dag: true` to its frontmatter. Then each `### Step N` (= a node) declares `id:` (stable, e.g. `N3`) and `depends: [N1, N2]` (multi-predecessor; absent ⇒ root). `/ae:work` then advances by the **ready-set frontier** (a node runs only when all its `depends:` are `pass`, verdict re-derived from disk) instead of document order — so a long task advances on dependency order. Each node keeps its existing deliverable (`Expected files:`) + harness (`verify_by`). `check-dag.sh validate` checks the graph (acyclic, no dangling depends, every auto-node has a runnable check); `/ae:plan-review` runs it as a gate. An optional node `backend:` may be `subagent` or `agent_teams`. A plan WITHOUT `dag: true` is a normal linear plan (unchanged). Honest bounds: the DAG drive is **serial** (one node at a time) and the DAG is fixed at plan time; it makes long-task advancement autonomous + dependency-ordered, but does not by itself guarantee semantic correctness of the work.
