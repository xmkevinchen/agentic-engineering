...
- **Runnable-check mandate (HDD)**: every **deterministic** `verify_by` AC (`unit`/`integration`/`e2e`/`contract`) MUST declare a runnable check — a `verify:` line naming the command (e.g. `verify: sh tests/x.sh`) OR a covering `test.command` target. See [`docs/references/verify-by-kinds.md`](../../../../docs/references/verify-by-kinds.md) for what a runnable check looks like per kind. `/ae:plan-review` blocks a deterministic AC with no runnable check (a deterministic claim with nothing to run is vacuous). `judge`/`manual` ACs are exempt (artifact/human, not shell).
