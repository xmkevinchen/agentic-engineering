...
- **WAIVED AC** (any `verify_by`): an AC whose body states it is WAIVED → **the reviewer MUST grep `<milestone-dir>/notes.md` to confirm a matching waiver reason is actually recorded there** (symmetry with the `UNVERIFIED_AC` grep above — challenger Ch3 fixup). A waiver record present → accept the waiver as satisfying the harness for that AC (the recorded reason is the audit trail; closes the gated-deferral gap for deterministic/`contract` ACs that legitimately did not run). A bare "WAIVED" in the AC body with **no** matching notes.md record → treat as unverified → **block verdict pass** (a prose-only waiver is exactly the silent-drop this backstop catches; do not take the AC body's word for it).
