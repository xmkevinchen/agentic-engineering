...
- **`node_check:` step (F-051)**: if a step declares `node_check:` lines, they are a declared/satisfied harness artifact — `check-node.sh` already gated advancement on them during `/ae:work` (red-before-green probe + real run each iteration). The reviewer confirms the relevant `node_check` instances are present and that their params plausibly exercise the deliverable (the param-selection-theater residual is bounded here + at plan-review, never fully closed); a step claiming a deterministic content guarantee with a vacuous/always-passing `node_check` → treat as unverified (P1 path).
- **`project` node_check cross-family backstop (F-052)**: a `node_check` whose lifecycle is `project` (its step has ANY `fixture: project` AC — conservative rule, F-052 integration) shipped WITHOUT a recorded cross-family bite-review → treat as unverified (P1 path). The record is keyed (mirrors the `WAIVED_AC <id>:` discipline): grep `<milestone-dir>/notes.md` for `NODE_CHECK_BITE_REVIEW Step <N>:` with a `verdict=bites` for that step — a generic "cross-family review done" mention does NOT satisfy it (a mis-keyed/prose-only record is the silent-drop this backstop catches). This is the review-stage backstop if plan-review's escalation was skipped, mirroring Check 7's role for `verify_by`. `per-feature` node_checks need no cross-family review (redcheck-only; cost scales with stakes).
- **Judge AC** (`verify_by: judge`): the reviewer answers the AC's stated rubric question against the actual output; a `judge` AC with no rubric, or one whose rubric fails → **block verdict pass**. (This is the committed `judge` enforcement path — review-stage rubric-confirmation; no separate engine.) This path covers **non-code dimensions — business-data validity, domain invariants, BDD/behavioral scenarios** — when the AC declares `verify_by: judge` with a rubric.
- **`manual` AC**: surfaced for human confirmation in the verdict section; not auto-blocking.
- **WAIVED AC** (any `verify_by`): an AC whose body states it is WAIVED → the reviewer MUST grep `<milestone-dir>/notes.md` for a line **`WAIVED_AC <this-AC-id>: <reason>`** — a waiver KEYED to this specific AC id (codex P2: a generic deferred-finding `Disposition: WAIVED: <reason>` line is NOT an AC waiver — it has no AC id, so an unrelated finding's waiver must not satisfy an AC). `WAIVED_AC <this-AC-id>:` present → accept the waiver for that AC (the recorded reason is the audit trail; closes the gated-deferral gap for deterministic/`contract` ACs that legitimately did not run). No keyed `WAIVED_AC <this-AC-id>` line → treat as unverified → **block verdict pass** (a prose-only or mis-keyed waiver is the silent-drop this backstop catches). The canonical AC-waiver write format is `WAIVED_AC <AC-id>: <reason>` — distinct from the deferred-finding `Disposition: WAIVED:` form.
