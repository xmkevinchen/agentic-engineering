...
- **WAIVED AC** (any `verify_by`): an AC whose body explicitly states it is WAIVED with a deferral reason recorded in `<milestone-dir>/notes.md` (e.g. a conditionally-gated AC whose precondition resolved no-go) → **accept the waiver as satisfying the harness for that AC** (do not block). The recorded reason is the audit trail. This closes the gated-deferral gap for deterministic/`contract` ACs that legitimately did not run.
