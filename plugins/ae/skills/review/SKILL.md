...
- **`project` node_check cross-family backstop (F-052)**: a `node_check` whose lifecycle is `project` (committable, durable) shipped WITHOUT a recorded cross-family bite-review (plan-review item (e)) → treat as unverified (P1 path) — this is the review-stage backstop if plan-review's cross-family escalation was skipped, mirroring Check 7's role for `verify_by`. `per-feature` node_checks need no cross-family review (redcheck-only; cost scales with stakes).
