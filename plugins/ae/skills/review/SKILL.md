...
- **Judge AC** (`verify_by: judge`): the reviewer evaluates a **captured artifact** at `<milestone-dir>/artifacts/<AC-id>.<ext>` (screenshot/log/metric/sample output) against the AC's stated rubric — the judge evaluates **THE ARTIFACT, never the executor's self-report**. A `judge` AC with **no rubric**, **no artifact at the expected path**, or a **failing rubric** → **block verdict pass**. (Committed `judge` enforcement — review-stage rubric-confirmation against a real artifact; no separate engine.) Covers **non-code dimensions — business-data validity, domain invariants, BDD/behavioral scenarios**.
