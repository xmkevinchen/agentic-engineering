...
10. **Query record (durable, append-only)**: append exactly ONE `query:` record to `.ae/graph/log.md`:
   ...
   The `query:` actor token is reserved and disjoint from `check:` / `add-page:` / `add-edges:` / `backfill:` / `dedup:` (record kinds share one ledger, selectable by stable prefix). Prior log bytes are never touched — append only.
