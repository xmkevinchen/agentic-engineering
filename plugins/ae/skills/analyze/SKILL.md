...
9. **Write-back (optional)**: when this locate-step produced a NOVEL exploration — understanding of a component/subsystem no existing `syn-*` page holds — it MAY be filed back as a NEW synthesis page via `plugins/ae/bin/graph-refresh.py add-page <page.json>` (new-page-only; the same evidence-bundle write gate and content contract as knowledge-refresh § Synthesis pages apply — every sentence anchored or explicitly marked as unanchored judgment). Skip freely: most lookups produce no page-worthy novelty.
