    ...
    for line in body.splitlines():
        ...
        if not stripped:
            if tldr_lines:
                break
            continue
        # markdown heading = #{1,6} + whitespace; "#1 priority" is prose
...
for existing in os.listdir(themes_dir):
    if existing.endswith(".md") and existing[:-3] not in slugs.values():
        ...
        # and never anything on a run that parsed zero records
...
def member_key(row):
    # numeric id sort — lexicographic puts F-1000 before F-932
