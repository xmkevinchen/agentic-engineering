#!/usr/bin/env python3
"""wiki-index-gen.py — deterministic layered index over feature frontmatter (F-069 Step 3).
...
Node discovery matches wiki-lint/discover_nodes: <root>/{active,done,abandoned,
...
Usage: wiki-index-gen.py [--root DIR] [--out DIR]
  --root  features root (default: $FEATURES_ROOT or .ae/features)
  --out   index output dir (default: .ae/wiki)
...
if __name__ != "__main__":
    raise SystemExit("wiki-index-gen.py is subprocess-only; do not import")
...
parser.add_argument("--out", default=".ae/wiki")
try:
...
if not os.path.isdir(root):
    print(f"[wiki-index-gen] usage error: no such root: {args.root}", file=sys.stderr)
    sys.exit(2)
...
for state in STATE_DIRS:
    ...
    for name in sorted(os.listdir(state_dir)):
        ...
        if data is None:
            print(f"[wiki-index-gen] skip {state}/{name}: {tldr}", file=sys.stderr)
            continue
        fid, title, status = data.get("id"), data.get("title"), data.get("status")
        if not fid or not title or not status:
            print(f"[wiki-index-gen] skip {state}/{name}: missing required id/title/status",
                  file=sys.stderr)
            continue
...
tier_a = ["# Project knowledge index — themes", "",
          "Generated by wiki-index-gen.py — deterministic aggregation of feature",
...
for theme in ordered:
    ...
    tier_b = [f"# Theme: {theme}", "",
              "Generated by wiki-index-gen.py — do not edit.", ""]
...
print(f"[wiki-index-gen] {len(ordered)} theme(s), {total} feature(s) → {out_dir}")
sys.exit(0)
