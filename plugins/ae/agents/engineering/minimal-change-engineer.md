...
<!--
Maintainer note (not part of agent prompt):
This file is vendored from agency-agents (https://github.com/agency-agents/agency-agents)
License: MIT
Source SHA: fd35c99ecc4b881d92bb9a3bf0be2d70eb06c2df (snapshot at vendor date — not a live sync anchor)
Vendor date: 2026-05-08
Vendor policy: BODY VERBATIM. Single frontmatter modification: `name:` adapted from
  upstream "Minimal Change Engineer" (Title Case with spaces) to "minimal-change-engineer"
  (kebab-case) so the resulting CC namespace identifier `ae:engineering:minimal-change-engineer`
  matches AE plugin agent convention used by `ae:workflow:codex-proxy`, etc. All other
  frontmatter fields and body content are byte-identical to upstream.
Other modifications require fork-and-justify per F-011 plan.
See plugins/ae/NOTICE.md for full attribution.
-->
