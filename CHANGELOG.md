...
- **F-072 — synthesis layer**: LLM-written architecture pages persisted under `.ae/graph/synthesis/` with machine-checkable anchors (`path:line` + whitespace-normalized line snapshot + optional commit cite; `graph-page-check.py` is the single checker — fresh/stale/DEFECT, STALE never blocks). Pull-gate freshness in all six Prior-Context locate-steps (stale flagged inline with the fix pointer, DEFECT never served); `add-page` machine write path (atomic, refuses divergent re-adds, deletes itself on a failed check); query write-back + append-only `.ae/graph/log.md`; opt-in `--log-validations` freshness telemetry; real-scale spike first (checks are O(anchors), repo-size-independent: 0.32 s whole-tree on a 12k-file repo). Shipped through the F-048 harness loop (2 iterations — the isolated judge failed the first bootstrap pages on unanchored prose).
- **F-073 — repo-wide jargon governance**: everything repo-entering (code comments, commit messages, skill prose, tests, docs) describes the work, never the review conversation. Stock cleaned (59-file comment sweep, 26 branch commit messages reworded); flow gated by `test-jargon-tripwire.sh` in the standard suite (its first run caught 12 leftovers the manual sweep missed).
- **Dual audit before merge** (implementation: Codex/gemma-26b/Claude isolated; corpus: cross-family ground-truth): fixed an edge self-anchoring hole (an evidence line pointing inside frontmatter could validate against its own inserted YAML), stale-at-write page acceptance, round-trip field loss in the new `remove-edges` delete path, and — the deep one — corrected confident-but-wrong claims on **4/4** bootstrap pages that the form-rubric judge had passed: batch re-judging is now the refresh DEFAULT (LLM output is never settled fact; `written_by: human` is the only exemption, enforced at the script layer), and page re-judging calls for a cross-family ground-truth pass, not same-family form review.
